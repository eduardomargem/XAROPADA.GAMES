...
function configurarEdicaoCamposPessoais() {
    // Remove atributo readonly (se existir) para permitir edição
...
function configurarAlteracaoSenha() {
    ...
    // Altera placeholder para indicar que é opcional
    senhaInput.placeholder = "Nova senha (deixe em branco para manter a atual)";
    confirmarSenhaInput.placeholder = "Confirme a nova senha";
    
    // Remove required se estiver presente
    senhaInput.removeAttribute('required');
    confirmarSenhaInput.removeAttribute('required');
}

function aprimorarEnderecosEntrega() {
    const btnAdicionar = document.getElementById('adicionar-endereco');
    
    // Adiciona feedback visual ao adicionar endereço
    btnAdicionar.addEventListener('click', () => {
        setTimeout(() => {
            const enderecos = document.querySelectorAll('.endereco-entrega');
            if (enderecos.length > 1) {
                enderecos[enderecos.length - 1].scrollIntoView({ behavior: 'smooth' });
            }
        }, 100);
    });
}
...
function inicializarFormulario() {
    ...
    // Configurações dos critérios de aceite (novas)
    configurarEdicaoCamposPessoais();
    configurarAlteracaoSenha();
    aprimorarEnderecosEntrega();
